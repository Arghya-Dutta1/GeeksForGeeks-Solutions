...
class Solution {
  public:
    ...
    Node* rebuild(vector<int>& pre, int pst, int pend, vector<int>& in, int ist, int iend, map<int, int>& mp){
        if(pst > pend || ist > iend)
            return NULL;
        
        Node* root=new Node(pre[pst]);
        int indr=mp[root->data];
        int noleft=indr-ist;
    ...
    Node *buildTree(vector<int> &inorder, vector<int> &preorder) {
        ...
        return rebuild(preorder, 0, n-1, inorder, 0, n-1, mp);
    }
};
